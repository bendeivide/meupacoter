...
A estrutura básica é que cada linha se inicia com a opção seguida de ":", e em seguida o valor. Segundo o manual [Writing R extensions](https://cran.r-project.org/doc/manuals/r-devel/R-exts.html#The-DESCRIPTION-file), os campos `Package`, `Version`, `Licence`, `Description`, `Title`, `Author` e `Mantainer` são obrigatórios .Todos os outros campos são opcionais. Todo o *DESCPTION* deve ser escrito em ASCII, isto é, caracteres sendo letras e números. Caso use acentuações, ou outros caracteres, deve ser acrescentado o campo `Encoding` e informar o tipo de codificação. No pacote [meupacoter](https://bendeivide.github.io/meupacoter/), por exemplo, foi utilizado `Encoding: UTF-8`, devido aos acentos na língua portuguesa. 
...
Description: O que o pacote faz (one paragraph).
License: Qual a licença que você usa
```


Iremos discutir a ideia sobre algumas dessas opções a seguir.

### Nome do pacote

A discução sobre o nome do pacote foi realizado na subseção [Escolha do nome e logo do pacote](introd_pkg.html#name_logo_pkg). O campo para o nome é `Package` e deve conter somente letras, números e ponto, e ter pelo menos dois caracteres, começar com uma letra e não terminar com um ponto. Esse campo não deve ser utilizado para explicar o que o pacote faz. Para isso use o campo `Description`.
